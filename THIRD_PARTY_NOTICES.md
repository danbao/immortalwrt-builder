...
| ImmortalWrt shellsync | https://github.com/immortalwrt/immortalwrt/tree/cf234f8de6d5a7a3fa38fa0f7a936a0d8b15856c/package/network/services/shellsync | GPL-2.0-only under repository `COPYING` |
...
ImmortalWrt 24.10.6 package metadata omits a `License` field for `shellsync`, which is installed as a hard dependency of `ppp`. The reviewed source has no separate license declaration, while the same fixed repository commit's [`COPYING`](https://github.com/immortalwrt/immortalwrt/blob/cf234f8de6d5a7a3fa38fa0f7a936a0d8b15856c/COPYING) declares GPL-2.0-only and states that all contributions are subject to it. The override is limited to package versions matching `0.2-*`; a version change blocks publication pending review.
