...
2. **Third-party packages** — feeds appended to `repositories.conf` (nikki/momo `*.pages.dev`, passwall2 SourceForge; signature checking disabled); release ipks downloaded into `packages/` (sbwml MosDNS offline bundle — chosen over the kiddin9 dl.openwrt.ai feed which intermittently fails from runners; asvow `luci-app-tailscale`). BBR enabled via `files/` sysctl overlay.
3. **Convert to OVA** — reuses `scripts/openwrt_img_to_ova.py scan` against `build-out/`.
4. **Publish** — `scripts/publish_releases.py` creates the Release (skips existing tags, so re-runs are safe), then `gh release upload` attaches the raw `.img.gz`.
5. **Record** — manifest + docs committed back to the branch.
...
1. Decompress `.img.gz` → raw image (gzip exit code 2 is tolerated for trailing garbage).
2. `qemu-img convert` to streamOptimized IDE VMDK.
3. Generate OVF (vmx-17, 2 vCPU/2GB, 1 VmxNet3 NIC via `--nic-count 1`; adjust the script flag for more) + SHA256 manifest, tar them into an OVA, write an `.ova.sha256` checksum.

### Dedup / idempotency

Conversion key = `image_sha256:BUILDER_VERSION`. Keys live in `manifests/converted-images.json`; `scan` skips keys already present. **Bump `BUILDER_VERSION` in `scripts/openwrt_img_to_ova.py` whenever conversion logic changes.**

### Runtime notes

- Nikki and Momo are both installed but mutually exclusive at runtime (conflicting transparent-proxy nft rules).
